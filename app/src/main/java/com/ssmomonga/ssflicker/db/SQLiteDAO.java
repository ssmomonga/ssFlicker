...
import com.ssmomonga.ssflicker.db.SQLiteDBH.AppCacheTableColumnName8;
import com.ssmomonga.ssflicker.db.SQLiteDBH.AppTableColumnName8;
import com.ssmomonga.ssflicker.db.SQLiteDBH.PointerTableColumnName8;
 ...
 */
	...
	public Pointer[] selectPointerTable() {
		...
		while (c.moveToNext()) {
			int pointerId = c.getInt(c.getColumnIndex(PointerTableColumnName8.POINTER_ID));
	...
	public App[][] selectAppTable() {
		...
		while (c.moveToNext()) {
			int pointerId = c.getInt(c.getColumnIndex(AppTableColumnName8.POINTER_ID));
			int appId = c.getInt(c.getColumnIndex(AppTableColumnName8.APP_ID));
	...
	public int[][] selectAppWidgets() {
		...
		SQLiteDatabase db = sdbh.getReadableDatabase();
		String[] columns = { AppTableColumnName8.POINTER_ID, AppTableColumnName8.APP_ID };
		String selection =  AppTableColumnName8.APP_TYPE + "=" + App.APP_TYPE_APPWIDGET;
		Cursor c = db.query(SQLiteDBH.APP_TABLE_8, columns, selection, null, null, null, AppTableColumnName8.APPWIDGET_UPDATE_TIME);
		
		int[][] appWidgetList = new int[c.getCount()][2];
		while (c.moveToNext()) {
			int i = c.getPosition();
			appWidgetList[i][0] = c.getInt(c.getColumnIndex(AppTableColumnName8.POINTER_ID));
			appWidgetList[i][1] = c.getInt(c.getColumnIndex(AppTableColumnName8.APP_ID));
		}
 ...
 */
	...
	private boolean checkAppId (int pointerId, int appId) {
		...
		if (pointerId >= 0 && pointerId < Pointer.FLICK_POINTER_COUNT && appId >= 0 && appId < App.FLICK_APP_COUNT) {
			...
			String[] columns = new String[] {PointerTableColumnName8.POINTER_ID};
			String selection = PointerTableColumnName8.POINTER_ID + "=" + pointerId;
			SQLiteDatabase db = sdbh.getReadableDatabase();
 ...
 */
	...
	public void deletePointerTable (int pointerId) {
		...
		long result = -1;
		String appTableWhereClause = AppTableColumnName8.POINTER_ID + "=?";
		String[] appTableWhereArgs = { String.valueOf(pointerId) };
		String pointerTableWhereClause = PointerTableColumnName8.POINTER_ID + "=?";
	...
	public void deleteAppTable (int pointerId, int appId) {
		...
		long result = -1;
		String appTableWhereClause = AppTableColumnName8.POINTER_ID + "=? and " + AppTableColumnName8.APP_ID + "=?";
	...
	private int[] selectAppWidgetIds (int pointerId, int appId) {
		...
		String selection;
		if (appId == -1) {
			selection = AppTableColumnName8.POINTER_ID + "=" + pointerId;
		} else {
			selection = AppTableColumnName8.POINTER_ID + "=" + pointerId + " and " + AppTableColumnName8.APP_ID + "=" + appId;
		}
		
		SQLiteDatabase db = sdbh.getReadableDatabase();
		Cursor c = db.query(SQLiteDBH.APP_TABLE_8, new String[] { AppTableColumnName8.APPWIDGET_ID }, selection, null, null, null, null);
		
		int[] appWidgetIds = new int[c.getCount()];
		int i = 0;
		while (c.moveToNext()) {
			int appWidgetId = c.getInt(c.getColumnIndex(AppTableColumnName8.APPWIDGET_ID));
	...
	public void _deleteErrorData() {
		...
		for (int i = 0; i < Pointer.FLICK_POINTER_COUNT + Pointer.DOCK_POINTER_COUNT; i ++) {
			for (int j = 0; j < App.FLICK_APP_COUNT; j ++) {
				...
				String selection = AppTableColumnName8.POINTER_ID + "=" + i + " and " + AppTableColumnName8.APP_ID + "=" + j;
				SQLiteDatabase db = sdbh.getReadableDatabase();
				Cursor c = db.query(SQLiteDBH.APP_TABLE_8, new String[] { AppTableColumnName8.APPWIDGET_ID }, selection, null, null, null, null);
				boolean b = false;
 ...
 */
	...
	public void editPointerTable (int pointerId, Pointer pointer) {
		...
		String whereClause = PointerTableColumnName8.POINTER_ID + "=?";
	...
	public void editAppTable(int pointerId, int appId, App app) {
		...
		String whereClause = AppTableColumnName8.POINTER_ID + "=? and " + AppTableColumnName8.APP_ID + "=?";
	...
	public void updateAppWidgetUpdateTime (int appWidgetId, long appWidgetUpdateTime) {
		...
		String whereClause = AppTableColumnName8.APPWIDGET_ID + "=?";
		String[] whereArgs = { String.valueOf(appWidgetId) };
		
		ContentValues cv = new ContentValues();
		cv.put(AppTableColumnName8.APPWIDGET_UPDATE_TIME, appWidgetUpdateTime);
 ...
 */
	...
	public void sortPointerTable (int fromPointerId, int toPointerId) {
		...
		cv[0].put(PointerTableColumnName8.POINTER_ID, -1);
		whereClause[0] = PointerTableColumnName8.POINTER_ID + "=?";
		whereArgs[0] = new String[]{ String.valueOf(toPointerId) };
		cv[1].put(AppTableColumnName8.POINTER_ID, -1);
		whereClause[1] = AppTableColumnName8.POINTER_ID + "=?";			
		whereArgs[1] = new String[]{ String.valueOf(toPointerId) };
		cv[2].put(PointerTableColumnName8.POINTER_ID, toPointerId);
		whereClause[2] = PointerTableColumnName8.POINTER_ID + "=?";
		whereArgs[2] = new String[]{ String.valueOf(fromPointerId) };
		cv[3].put(AppTableColumnName8.POINTER_ID, toPointerId);
		whereClause[3] = AppTableColumnName8.POINTER_ID + "=?";
		whereArgs[3] = new String[]{ String.valueOf(fromPointerId) };
		cv[4].put(PointerTableColumnName8.POINTER_ID, fromPointerId);
		whereClause[4] = PointerTableColumnName8.POINTER_ID + "=?";
		whereArgs[4] = new String[]{ "-1" };
		cv[5].put(AppTableColumnName8.POINTER_ID, fromPointerId);
		whereClause[5] = AppTableColumnName8.POINTER_ID + "=?";
	...
	public void sortAppTable (int pointerId, int fromAppId, int toAppId) {
		...
		cv[0].put(AppTableColumnName8.APP_ID, -1);
		whereClause[0] = AppTableColumnName8.POINTER_ID + "=? and " + AppTableColumnName8.APP_ID + "=?";
		whereArgs[0] = new String[]{ String.valueOf(pointerId), String.valueOf(toAppId) };
		cv[1].put(AppTableColumnName8.APP_ID, toAppId);
		whereClause[1] = AppTableColumnName8.POINTER_ID + "=? and " + AppTableColumnName8.APP_ID + "=?";
		whereArgs[1] = new String[]{ String.valueOf(pointerId), String.valueOf(fromAppId) };
		cv[2].put(AppTableColumnName8.APP_ID, fromAppId);
		whereClause[2] = AppTableColumnName8.POINTER_ID + "=? and " + AppTableColumnName8.APP_ID + "=?";
 ...
 */
	...
	private Drawable createMultiAppsIcon(int pointerId) {
		...
		SQLiteDatabase db = sdbh.getReadableDatabase();
		String selection = AppTableColumnName8.POINTER_ID + "=" + pointerId;
 ...
 */
	...
	private Pointer createPointer (Cursor c) {
		...
		return new Pointer(
				c.getInt(c.getColumnIndex(PointerTableColumnName8.POINTER_TYPE)),
				c.getString(c.getColumnIndex(PointerTableColumnName8.POINTER_LABEL)),
				ImageConverter.createDrawable(context, c.getBlob(c.getColumnIndex(PointerTableColumnName8.POINTER_ICON))),
				c.getInt(c.getColumnIndex(PointerTableColumnName8.POINTER_ICON_TYPE)),
				c.getInt(c.getColumnIndex(PointerTableColumnName8.POINTER_ICON_TYPE_APP_APP_ID))
				);
	}
	...
	private App createApp (Cursor c) {
		...
		int AppType = c.getInt(c.getColumnIndex(AppTableColumnName8.APP_TYPE));
		
		switch (AppType) {
			case App.APP_TYPE_INTENT_APP:
				return new App(
						context,
						c.getInt(c.getColumnIndex(AppTableColumnName8.APP_TYPE)),
						c.getString(c.getColumnIndex(AppTableColumnName8.PACKAGE_NAME)),
						c.getString(c.getColumnIndex(AppTableColumnName8.APP_LABEL)),
						c.getInt(c.getColumnIndex(AppTableColumnName8.APP_LABEL_TYPE)),
						ImageConverter.createDrawable(context, c.getBlob(c.getColumnIndex(AppTableColumnName8.APP_ICON))),
						c.getInt(c.getColumnIndex(AppTableColumnName8.APP_ICON_TYPE)),
						new IntentAppInfo(c.getInt(c.getColumnIndex(AppTableColumnName8.INTENT_APP_TYPE)),
								c.getString(c.getColumnIndex(AppTableColumnName8.INTENT_URI))));
			...
			case App.APP_TYPE_APPWIDGET:
				return new App(
						...
						new AppWidgetInfo(context,
								c.getInt(c.getColumnIndex(AppTableColumnName8.APPWIDGET_ID)),
								c.getInt(c.getColumnIndex(AppTableColumnName8.APPWIDGET_CELL_POSITION_X)),
								c.getInt(c.getColumnIndex(AppTableColumnName8.APPWIDGET_CELL_POSITION_Y)),
								c.getInt(c.getColumnIndex(AppTableColumnName8.APPWIDGET_CELL_WIDTH)),
								c.getInt(c.getColumnIndex(AppTableColumnName8.APPWIDGET_CELL_HEIGHT)),
								c.getLong(c.getColumnIndex(AppTableColumnName8.APPWIDGET_UPDATE_TIME))));
			...
			case App.APP_TYPE_FUNCTION:
				return new App(
						...
						ImageConverter.createDrawable(context, c.getBlob(c.getColumnIndex(AppTableColumnName8.APP_ICON))),
						c.getInt(c.getColumnIndex(AppTableColumnName8.APP_ICON_TYPE)),
						new FunctionInfo(c.getInt(c.getColumnIndex(AppTableColumnName8.FUNCTION_TYPE))));
	...
	private App createAppCache (Cursor c) {
		...
		return new App(
				...
				c.getString(c.getColumnIndex(AppCacheTableColumnName8.PACKAGE_NAME)),
				c.getString(c.getColumnIndex(AppCacheTableColumnName8.APP_LABEL)),
				IconList.LABEL_ICON_TYPE_ACTIVITY,
				ImageConverter.createDrawable(context, c.getBlob(c.getColumnIndex(AppCacheTableColumnName8.APP_ICON))),
				IconList.LABEL_ICON_TYPE_ACTIVITY,
				new IntentAppInfo(IntentAppInfo.INTENT_APP_TYPE_LAUNCHER,
						c.getString(c.getColumnIndex(AppCacheTableColumnName8.INTENT_URI))));
	}
 ...
 */
	...
	private ContentValues createAppCacheCV (App app) {
		...
		cv.put(AppCacheTableColumnName8.PACKAGE_NAME, app.getPackageName());
		cv.put(AppCacheTableColumnName8.APP_LABEL, app.getAppLabel());
		cv.put(AppCacheTableColumnName8.APP_ICON, ImageConverter.createByte(context, app.getAppIcon()));
		cv.put(AppCacheTableColumnName8.INTENT_URI, app.getIntentAppInfo().getIntentUri());
