package com.ssmomonga.ssflicker.settings;
...
//	public static final String CHANGED_PACKAGE_SEQUENCE = "changed_package_sequence";
	...
	private void checkPrefVersion() {
		...
		if (oldVersion < PREF_VERSION_NUM) {
			switch(oldVersion) {
				case 1:
		    ...
		    Editor editor = prefs.edit();
		    editor.putInt(PREF_VERSION, PREF_VERSION_NUM);
		    editor.commit();
		}
	}
	 ...
	 * isOverlay()
	 *
	 * オーバーレイ起動が有効化されているか確認する。
	 * オーバーレイポイント1または2のいずれかが有効化されている場合はtrueを返却する。
	 *
	 * @return
	 */
	...
	public int getOverlayPointWidth(int overlayPointNumber) {
		return DeviceSettings.dpToPixel(
				context,
				Integer.parseInt(getRawOverlayPointWidth(overlayPointNumber)));
	}
	...
	public int getOverlayPointBackgroundColor() {
		return prefs.getInt(OVERLAY_POINT_BACKGROUND_COLOR,
				context.getColor(R.color.overlay_point_background_color_default_value));
	}
	...
	public int getWindowBackgroundColor() {
		return prefs.getInt(WINDOW_BACKGROUND_COLOR,
				context.getColor(R.color.window_background_color_default_value));
	}
	...
	public int getTextColor() {
		return prefs.getInt(TEXT_COLOR, context.getColor(android.R.color.white));
	}
