...
import com.ssmomonga.ssflicker.params.ColorPickerParams;
import com.ssmomonga.ssflicker.settings.DeviceSettings;

/**
 * ColorPickerDialog
 */
public abstract class ColorPickerDialog extends AlertDialog {
	
	public static final int COLOR_TYPE_OVERLAY_POINT_BACKGROUND = 0;
	public static final int COLOR_TYPE_WINDOW_BACKGROUND = 1;
	public static final int COLOR_TYPE_TEXT = 2;
	public static final int COLOR_TYPE_ICON = 3;
	...
	private ColorPickerParams settings;
	 ...
	 * @param colorType
	 */
	public ColorPickerDialog(Context context, int colorType) {
		super(context);
		this.colorType = colorType;
		settings = new ColorPickerParams(context);
		setInitialLayout(R.layout.color_picker_dialog_light);
	}
	 ...
	 * @param iconColor
	 */
	public ColorPickerDialog(Context context, int colorType, int iconColor) {
		super(context);
		this.colorType = colorType;
		settings = new ColorPickerParams(context, iconColor);
		setInitialLayout(R.layout.color_picker_dialog_dark);
	}
	...
	private void setInitialLayout(int res) {
		...
		int[] colorPallet = getContext().getResources().getIntArray(R.array.color_pallet);
		for (int i = 0; i < COLOR_PALLET_COUNT; i ++) {
			...
			iv_color_pallet[i].setOnClickListener(new View.OnClickListener() {
				@Override
		...
		Drawable wallpaper = DeviceSettings.getWallpaper(getContext());
		ll_wallpaper.setBackground(wallpaper);
		for (int i = 0; i < PREVIEW_COUNT; i ++) {
			ll_window_preview[i].setLayoutParams(settings.getAppLP());
			...
			if (settings.isTextVisibility()) {
				...
				tv_pointer_preview[i].setVisibility(View.GONE);
			}
			if (settings.isOverlay()) {
				ll_overlay_point_preview[i].setVisibility(View.VISIBLE);
				ll_overlay_point_preview[i].setBackgroundColor(
						settings.getOverlayPointBackgroundColor());
			} else {
				ll_overlay_point_preview[i].setVisibility(View.GONE);
			}
		}
	...
	private void setPreview(int i) {
		switch (colorType) {
			case COLOR_TYPE_WINDOW_BACKGROUND:
				settings.setWindowBackgroundColor(
						Color.argb(colorARGB[0], colorARGB[1], colorARGB[2], colorARGB[3]));
				ll_window_preview[i].setBackground(settings.getWindowBackground());
				break;
			case COLOR_TYPE_ICON:
				settings.setIconColor(
						Color.argb(colorARGB[0], colorARGB[1], colorARGB[2], colorARGB[3]));
				iv_pointer_preview[i].setColorFilter(settings.getIconColor());
				iv_pointer_preview[i].setImageDrawable(settings.getIcon());
				break;
			case COLOR_TYPE_TEXT:
				...
				break;
			case COLOR_TYPE_OVERLAY_POINT_BACKGROUND:
				settings.setOverlayPointBackgroundColor(
						Color.argb(colorARGB[0], colorARGB[1], colorARGB[2], colorARGB[3]));
				ll_overlay_point_preview[i].setBackgroundColor(
						settings.getOverlayPointBackgroundColor());
				break;
	 ...
	 * FocusChengeListener
	 */
	private class FocusChengeListener implements View.OnFocusChangeListener {
